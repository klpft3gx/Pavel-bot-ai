...
# Настройка Groq-клиента (он совместим с OpenAI)
client = AsyncOpenAI(
    base_url="https://api.groq.com/openai/v1",
    api_key=GROQ_API_KEY,
)

# Модель, которую будем использовать (можно заменить на "llama3-70b-8192")
MODEL_NAME = "llama3-8b-8192"  # Очень быстрая и бесплатная

# Системный промпт – ОСНОВА ВАШЕЙ ЛИЧНОСТИ БОТА
SYSTEM_PROMPT = """
...
# Хранилище истории диалогов (для каждого пользователя отдельно)
# В реальном проекте лучше использовать Redis или БД, но для локального теста пойдёт словарь
user_contexts = {}
...
# Команда /clear – очистка истории для пользователя
...
async def handle_message(message: types.Message):
    ...
    # Если истории ещё нет, создаём её с системным промптом
    ...
    try:
        ...
        response = await client.chat.completions.create(
            model=MODEL_NAME,
            messages=context,
            temperature=0.9,    # креативность (0 – строго по факту, 1 – более живая речь)
